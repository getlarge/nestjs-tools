...
describe('buildWwwAuthenticateHeader', () => {
  ...
  it('throws when authorization is disabled', () => {
    expect(() => buildWwwAuthenticateHeader({ enabled: false })).toThrow(/disabled/i);
  });
});
...
describe('mountWellKnownRoutes on Fastify', () => {
  ...
  beforeAll(async () => {
    ...
    mountWellKnownRoutes(new FastifyMcpAdapter(app.getHttpAdapter() as never), authConfig);
    await app.listen(0);
  });
...
describe('mountWellKnownRoutes on Express', () => {
  ...
  beforeAll(async () => {
    app = await NestFactory.create<NestExpressApplication>(EmptyModule, new ExpressAdapter(express()));
    mountWellKnownRoutes(new ExpressMcpAdapter(app.getHttpAdapter() as never), authConfig);
    await app.listen(0);
  });
...
describe('mountWellKnownRoutes when authorization is disabled', () => {
  it('is a no-op', async () => {
    const app = await NestFactory.create<NestFastifyApplication>(EmptyModule, new FastifyAdapter());

...
import { MCP_PROMPT_METADATA, McpPromptMetadata } from '../decorators/mcp-prompt.decorator';
import { MCP_RESOURCE_METADATA, McpResourceMetadata } from '../decorators/mcp-resource.decorator';
import { MCP_TOOL_METADATA, McpToolMetadata } from '../decorators/mcp-tool.decorator';
...
export class McpDiscoveryService {
  ...
  private scan<TMetadata>(metadataKey: symbol): McpDecoratedMethod<TMetadata>[] {
  ...
  private scanInstance<TMetadata>(
    ...
    for (const methodName of this.metadataScanner.getAllMethodNames(prototype)) {
      const metadata = Reflect.getMetadata(metadataKey, prototype, methodName) as TMetadata | undefined;
  ...
  private isAddressable(wrapper: InstanceWrapper): wrapper is InstanceWrapper & { instance: object } {
    return !!wrapper.instance && typeof wrapper.instance === 'object' && !!wrapper.metatype;
  }
}

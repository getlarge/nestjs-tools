...
function makeValidator(outcome: 'ok' | 'invalid', payload?: Record<string, unknown>): TokenValidator {
  return {
    validateToken: jest
      .fn()
      .mockResolvedValue(
...
function makeContext(opts: { token?: string; toolName?: string; scopes?: string[] }): ExecutionContext {
  class FakeTools {
...
describe('McpAuthGuard scope enforcement', () => {
  it('rejects with Forbidden when @McpScopes is set and token lacks the scope', async () => {
    const guard = new McpAuthGuard(config, makeValidator('ok', { sub: 'alice', scope: 'tools:read' }), reflector);
  ...
  it('passes when token carries all required scopes (scope array)', async () => {
    const guard = new McpAuthGuard(config, makeValidator('ok', { sub: 'alice', scopes: ['tools:write'] }), reflector);
    const ctx = makeContext({ token: 'good', scopes: ['tools:write'] });
    await expect(guard.canActivate(ctx)).resolves.toBe(true);
  });
});

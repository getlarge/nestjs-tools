...
export function transformException(err: Error & { code: string }): HttpException;
export function transformException(err: Error | undefined): Error | undefined | HttpException {
  if (!err || err instanceof HttpException || !('code' in err) || typeof err.code !== 'string') {
    return err;
  }

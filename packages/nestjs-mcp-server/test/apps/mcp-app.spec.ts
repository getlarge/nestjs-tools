...
class FakeMcpServer {
  ...
  registerTool(name: string, config: Record<string, unknown>, callback: RegisteredTool['callback']): unknown {
...
describe('@McpApp decorator', () => {
  it('stores app metadata next to @McpTool', () => {
    const meta = Reflect.getMetadata(MCP_APP_METADATA, WeatherTools.prototype, 'forecast') as
      | McpAppMetadata
      | undefined;
  ...
  it('throws when neither html, file nor url is provided', () => {
    expect(() => McpApp({ uri: 'ui://x' })).toThrow(/html|file|url/i);
  });
...
beforeAll(async () => {
  moduleRef = await Test.createTestingModule({
    ...
    providers: [Reflector, McpDiscoveryService, McpPipelineRunner, McpToolRegistrar, McpAppRegistrar, WeatherTools],
  }).compile();
...
describe('McpAppRegistrar tool _meta weaving', () => {
  it('annotates the tool result with _meta referencing the ui resource', async () => {
    ...
    const result = (await tool!.callback({ city: 'Paris' }, { authInfo: {}, sessionId: 's' })) as {
